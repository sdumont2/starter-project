...
// Adding Environment for property resolving in readers
public class SpringLocalContainerEntityManagerFactoryBean extends AbstractEntityManagerFactoryBean
        implements ResourceLoaderAware, LoadTimeWeaverAware {

    @Nullable
    private PersistenceUnitManager persistenceUnitManager;

    private final ResolvingDefaultPersistenceUnitManager internalPersistenceUnitManager = new ResolvingDefaultPersistenceUnitManager();

    @Nullable
    private PersistenceUnitInfo persistenceUnitInfo;

    public SpringLocalContainerEntityManagerFactoryBean(Environment environment) {
        internalPersistenceUnitManager.setEnvironment(environment);
    }
    ...
    public void afterPropertiesSet() throws PersistenceException {
        ...
        if (this.entityManagerFactoryInterface == null) {
            ...
            if (!ClassUtils.isVisible(this.entityManagerFactoryInterface, getBeanClassLoader())) {
    ...
    protected EntityManagerFactory createEntityManagerFactoryProxy(@Nullable EntityManagerFactory emf) {
        ...
        try {
            return (EntityManagerFactory) Proxy.newProxyInstance(getBeanClassLoader(),
                    ClassUtils.toClassArray(ifcs), new SpringLocalContainerEntityManagerFactoryBean.ManagedEntityManagerFactoryInvocationHandler(this));
        }
     ...
     *
     * Overridden from abstract to
     */
    public Object invokeProxyMethod(Method method, @Nullable Object[] args) throws Throwable {
        ...
        // Reason for overriding is to enable criteria builder for OGM
        if (method.getName().equals("getCriteriaBuilder")) {
            if ( SessionFactoryImpl.class.isAssignableFrom( getNativeEntityManagerFactory().getClass() ) ) {
                return new CriteriaBuilderImpl((SessionFactoryImpl) getNativeEntityManagerFactory());
            } else {
                if (SessionFactoryDelegatingImpl.class.isAssignableFrom(getNativeEntityManagerFactory().getClass())) {
                    Method getDelegateMethod = SessionFactoryDelegatingImpl.class.getDeclaredMethod("delegate");
                    getDelegateMethod.setAccessible(true);
                    return new CriteriaBuilderImpl((SessionFactoryImpl) getDelegateMethod.invoke(getNativeEntityManagerFactory()));
                }
            }
        }
    ...
    private static class ManagedEntityManagerFactoryInvocationHandler implements InvocationHandler, Serializable {
        ...
        private final SpringLocalContainerEntityManagerFactoryBean entityManagerFactoryBean;

        public ManagedEntityManagerFactoryInvocationHandler(AbstractEntityManagerFactoryBean emfb) {
            this.entityManagerFactoryBean = (SpringLocalContainerEntityManagerFactoryBean)emfb;
        }
